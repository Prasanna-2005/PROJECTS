...
class cells:
        def __init__(self, row, col):
                    self.cid = row * 7 + col
                    self.neighbours = []  # Top Right Bottom Left index: 0 1 2 3 
                    if self.cid == 0:
                        self.neighbours = [None, self.cid + 1,self.cid + 7,None]
                    elif self.cid == (GRID-1):
                        self.neighbours = [None,None,self.cid + 7, self.cid - 1]
                    elif self.cid == (GRID*(GRID-1)):
                        self.neighbours = [self.cid - 7, self.cid + 1,None,None]
                    elif self.cid == (GRID*GRID)-1:
                        self.neighbours = [self.cid - 7,None,None, self.cid - 1]
                    elif self.cid in range(1, GRID-1):
                        self.neighbours = [None,self.cid + 1, self.cid + 7, self.cid - 1]
                    elif self.cid in range(GRID, GRID*(GRID-1), GRID):
                        self.neighbours = [self.cid - 7, self.cid + 1, self.cid + 7,None]
                    elif self.cid in range(2*GRID-1, (GRID*GRID)-1, GRID):
                        self.neighbours = [self.cid - 7,None, self.cid +7, self.cid -1]
                    elif self.cid in range((GRID*(GRID-1)), (GRID*GRID)-1):
                        self.neighbours = [self.cid - 7, self.cid + 1, None,self.cid - 1]
                    else:
                         self.neighbours = [self.cid - 7,self.cid+7, self.cid + 1,self.cid-1]
                    
                    self.c_filled = 0
                    self.win =None  #takes R or G
                    self.win_color = None #takes 0 or 1
                    ...
                    self.edges= [
                          ...
                          [(self.left,self.bottom),(self.right,self.bottom)],  #bottom
                           [(self.left,self.bottom),(self.left,self.top)],  #left  
                          ]  
                    self.sides = [0,0,0,0]  #    top,right,bottom,left 0,1,2,3
        ...
        def cellfilled(mainwin,color):  #fill cell if got 4 edges
            ...
            for i in allcell:
              if 0 not in i.sides and not i.c_filled:
                    ...
                    x.refresh()
                    newfill = 1
              elif 0 not in i.sides and i.c_filled:
                    x = curses.newwin(2,5,i.top+1,i.left+1)
                     
                    x.bkgd(' ',curses.color_pair(i.win_color)| curses.A_REVERSE)
                    mainwin.refresh()
                    x.refresh()
            return newfill
        ...
        def set_edge_true(self,index,edge):  #updates edges on neighbour if current drawn edge is common to both
              edge1 = edge
              edge2 = [edge[1],edge[0]]
        ...
        def modify(self,window,color,which_side):  #red->1  green->1   draw hline and vline  
             ...
             for i,side in enumerate(self.sides):
                if(side and i==which_side):
                        ...
                        comp = cells.cellfilled(window,color)
...
def initiate_draw_edge(mainwin,i,edgeindex,color):
            ...
            box = i.modify(mainwin,color,edgeindex)   
            return box

def inspect(mainwin,cellid,color,next_click):
               f = ''
               box_filled = 0
               i = cells.give_obj_for_id(cellid)  #let i be cell obj
               if next_click == 443:
                     index = 3   #left edge
                     if(not i.sides[3]):
                        mainwin.addstr(i.cellmid_y,i.cellmid_x," ")
                        box_filled = initiate_draw_edge(mainwin,i,index,color)
                        x = box_filled
                     ...
                     if not(i.sides[2]):
                        mainwin.addstr(i.cellmid_y,i.cellmid_x," ")
                        x = box_filled = initiate_draw_edge(mainwin,i,index,color)
                        cellid = str(i.cid).zfill(2)
                        f=f"{x}"+f"{cellid}"+"2"
                        return (f,box_filled)
               elif next_click == 480: 
...
def enter_game_player_R(mainwin,client,my_color,oppo_color):#red starts game
    ...
    mainwin.refresh()
    while(not gameover()):
            current_id = 0  #cell id
            i = cells.give_obj_for_id(current_id)
            mainwin.addstr(i.cellmid_y,i.cellmid_x,".")
            mainwin.refresh()
            while(1):  #user turn
                ...
                if getclick == curses.KEY_UP:
                    direction = 0   #m denotes which edge top left bottom right
                    if(current_id - 7)>=0:
                        current_id = current_id -7
                        mainwin.addstr(i.cellmid_y,i.cellmid_x," ")
                        cells.cellfilled(mainwin,my_color)
                        i = cells.give_obj_for_id(current_id)
                        mainwin.addstr(i.cellmid_y,i.cellmid_x,".")

                elif(getclick == curses.KEY_DOWN):
                     direction = 2
                     if(current_id + 7)<=48:
                        current_id = current_id +7
                        mainwin.addstr(i.cellmid_y,i.cellmid_x," ")
                ...
                elif(getclick == curses.KEY_RIGHT):
                     direction = 1
                     if(current_id + 1)<=48:
                        current_id = current_id +1
                        mainwin.addstr(i.cellmid_y,i.cellmid_x," ")
                ...
                elif(getclick in [480,444,481,443]):  #\n or \r  because enter key interpreted like this
                     move = inspect(mainwin,current_id,1,getclick)
                     print(move)
                     if move[0]=='' and move[1] == 0:  #turn over                       
                        break
                     else:
                        if move[1] == 1:
                            client.send(move[0].encode())
                            continue
                        else:
                            client.send(move[0].encode())
                            break 
...
def enter_game_player_G(mainwin,client,my_color,oppo_color):#green starts by follow
        ...
        while(not gameover()):
            ...
            while(not gameover()):
                ...
                rightup.clear()
                rightup.refresh()
                rightup.bkgd(" ",curses.color_pair(2)|curses.A_REVERSE)
                rightup.refresh()
                ...
                if getclick == curses.KEY_UP:
                    ...
                    if(current_id - 7)>=0:
                     ...
                     move = inspect(mainwin,current_id,2,getclick)
                     print(move)
                     if move[0]=='' and move[1] == 0:  #turn over                       
                        break
                     else:
